...
# compression -- neither ships with the .NET runtime image, so both are installed here. This is a
# slow layer (apt + npm, several minutes on a cold cache) but Docker caches it across builds as
# long as this RUN instruction's text and the base image above don't change -- so avoid editing
# this block unless you actually need to (e.g. bumping the KTX-Software version); every unrelated
# app-code deploy will otherwise reuse the cached layer for free.
